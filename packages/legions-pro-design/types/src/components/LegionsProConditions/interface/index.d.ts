...
import { InputProps, DatePickerProps, RangePickerProps, InputNumberProps, RadioGroupProps, TextAreaProps, CheckboxGroupProps, CheckboxProps, IAntdSelectOption } from '../../interface/antd';
...
interface IMethods {
    ...
    /** 查询指定下拉组件数据项 */
    getQuerySelectOption: (/** 下拉组件name值 */ name: string, /** 下拉选项值*/ option_key: string) => {
        readonly curr_item: LegionsLabeledValue;
        readonly data: Array<ProSelect['options']>;
    };
    /** 下拉远程搜索
     * 主要同于手动触发下拉组件搜索函数 */
    onSelectRequest: (
...
interface IQueryProps extends IFieldsState {
    ...
    */
    ...
    /** 只读 */
    readonly onPaste?: (event: any) => void;
    /** 只读 */
    readonly onEnter?: (value: any, viewEntity?: IViewQueryConditionStore) => void;
}
...
export interface ProConditions {
    ...
    /** 配置类型集合 */
    config_types: 'text' | 'checkBox' | 'groupCheckBox' | 'rangePicker' | 'date' | 'select' | 'textNumber' | 'radioButton' | 'textArea' | 'search';
